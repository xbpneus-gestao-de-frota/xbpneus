...
export default function VeiculoEdit() {
  ...
  const loadVeiculo = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/transportador/frota/veiculos/${id}/`);
      ...
      setForm({
        ...
        km: data.km || "0",
        km_ultima_manutencao: data.km_ultima_manutencao || "0",
        km_proxima_manutencao: data.km_proxima_manutencao || "",
        motorista: data.motorista || "",
        chassi: data.chassi || "",
        renavam: data.renavam || "",
        capacidade_carga: data.capacidade_carga || "",
        numero_eixos: data.numero_eixos || "3",
        total_posicoes_pneus: data.total_posicoes_pneus || "6",
        data_aquisicao: data.data_aquisicao || "",
        observacoes: data.observacoes || ""
      });
    } catch (ex) {
      console.error("Erro ao carregar veículo:", ex);
      setError("Falha ao carregar dados do veículo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <PageHeader 
        ...
        subtitle={`Editando veículo ${form.placa}`}
      />
